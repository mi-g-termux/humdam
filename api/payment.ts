...
async function sslcommerzCreatePayment(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const form = new URLSearchParams({
      ...
      success_url: `${o}/api/sslcommerz/callback?status=success&orderId=${encodeURIComponent(oid)}`,
      fail_url:    `${o}/api/sslcommerz/callback?status=failed&orderId=${encodeURIComponent(oid)}`,
      cancel_url:  `${o}/api/sslcommerz/callback?status=cancelled&orderId=${encodeURIComponent(oid)}`,
